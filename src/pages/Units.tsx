...
const Units = () => {
  ...
  // Helpers for filters
  const getUniqueUnitNames = () => [...new Set(units.map(unit => unit.unit_name))];
  const getUniqueUnitTypes = () => [...new Set(units.map(unit => unit.unit_type))];
  ...
  const filteredUnits = units.filter(unit => {
    ...
    if (selectedFocusAreas.length > 0 && !selectedFocusAreas.some(f => focusAreaKeys.includes(f))) {
      return false;
    }
    return true;
  });
  return <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex">
        ...
        <div className="w-72 bg-white border-r border-gray-200 p-6 min-h-screen rounded-md">
          ...
          <div className="mb-6">
            ...
            {expandedSections.units && <div className="space-y-2">
                <div className="flex flex-wrap gap-2">
                  {getUniqueUnitNames().map(unitName => <span key={unitName} onClick={() => toggleFilter(unitName, setSelectedUnits)} className={`px-3 py-1 text-sm border rounded-full cursor-pointer ${selectedUnits.includes(unitName) ? 'bg-sky-500 text-white border-sky-500' : 'border-gray-300 text-gray-700 hover:bg-gray-100'}`}>
                      {unitName}
                    </span>)}
                </div>
              </div>}
          ...
          <div className="mb-6">
            ...
            {expandedSections.industry && <div className="space-y-2">
                <div className="flex flex-wrap gap-2">
                  {getUniqueUnitTypes().map(unitType => <span key={unitType} onClick={() => toggleFilter(unitType, setSelectedTypes)} className={`px-3 py-1 text-sm border rounded-full cursor-pointer ${selectedTypes.includes(unitType) ? 'bg-sky-500 text-white border-sky-500' : 'border-gray-300 text-gray-700 hover:bg-gray-100'}`}>
                      {unitType}
                    </span>)}
                </div>
              </div>}
          ...
          <div className="mb-6">
            ...
            {expandedSections.interest && <div className="space-y-2">
                <div className="flex flex-wrap gap-2">
                  {getUniqueFocusAreas().map(focusArea => <span key={focusArea} onClick={() => toggleFilter(focusArea, setSelectedFocusAreas)} className={`px-3 py-1 text-sm border rounded-full cursor-pointer ${selectedFocusAreas.includes(focusArea) ? 'bg-sky-500 text-white border-sky-500' : 'border-gray-300 text-gray-700 hover:bg-gray-100'}`}>
                      {focusArea}
                    </span>)}
                </div>
              </div>}
          ...
          {/* Apply + Clear */}
          <div className="space-y-2">
            <Button className="w-full text-sm font-medium py-2 border-2 border-teal-500 rounded-3xl bg-transparent hover:bg-teal-50 text-teal-500">
                Apply
              </Button>
            <Button variant="outline" className="w-full text-sm font-medium py-2 rounded-md" onClick={clearFilters}>
              Clear Filters
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            </div> : filteredUnits.length === 0 ? <div className="text-center py-16">
