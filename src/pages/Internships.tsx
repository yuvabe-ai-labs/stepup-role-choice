...
const Internships = () => {
  ...
  const uniqueDepartments = Array.from(new Set(internships.map((i) => i.title?.split(" ")[0]).filter(Boolean)));
      ...
      <div className="flex gap-6 p-6">
        ...
        <div className="w-80 bg-card pt-[20px] border rounded-3xl flex flex-col h-[90vh] sticky top-6">
          ...
          <div className="px-6 pb-6 overflow-y-auto flex-1 space-y-6">
            ...
            {/* Course Period */}
            <div>
              <Label className="text-sm font-semibold text-muted-foreground mb-3 block">Course Period</Label>
              ...
              <Slider
                ...
                onValueChange={([min, max]) => setFilters({ ...filters, coursePeriod: { min, max } })}
                min={0}
                max={12}
                step={1}
                className="mb-2"
                    ...
                    <Popover key={key}>
                      ...
                      <PopoverContent className="p-0" align="start">
                        <Calendar
                          ...
                          selected={
                            filters.postingDate[key as keyof typeof filters.postingDate]
                              ? new Date(filters.postingDate[key as keyof typeof filters.postingDate])
                              : undefined
                          }
                          onSelect={(date) =>
                            setFilters({
