...
const CreateInternshipDialog: React.FC<CreateInternshipDialogProps> = ({ isOpen, onClose, onSuccess }) => {
  const { user } = useAuth();
  ...
  const handleAIAssist = async (fieldName: keyof FormData) => {
    ...
    try {
      ...
      switch (fieldName) {
        case "description":
          prompt = `Write a clear and professional "About Internship" description for a ${jobTitle} internship position. The description should be 1-2 paragraphs explaining what the internship is about, what the intern will be doing. Make it engaging and suitable for all types of internship roles.${
            currentValue ? ` Current description: "${currentValue}". Please improve and rewrite it.` : ""
          }`;
          break;
        case "responsibilities":
          prompt = `Write 5-7 key "responsibilities" for a ${jobTitle} internship. Format as bullet points, one per line. Make them clear, actionable, and relevant to the role.${
            currentValue ? ` Current responsibilities: "${currentValue}". Please improve and expand on them.` : ""
          }`;
          break;
        case "benefits":
        ...
        case "skills_required":
          prompt = `List 5-8 essential skills required for a ${jobTitle} internship. Format as a comma-separated list. Include both technical and soft skills relevant to the role.${
            currentValue ? ` Current skills: "${currentValue}". Please improve and expand on them.` : ""
          }`;
          break;
        default:
      ...
      if (aiResponse?.response) {
        ...
        setConversationHistory((prevHistory) => [...prevHistory, { role: "ai", content: cleanResponse }]);
        ...
        toast({
          ...
          description: "Received unexpected response from AI. Please try again.",
          variant: "destructive",
        });
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <Input {...field} id="title" placeholder="Enter Job role" className="bg-background" />
                )}
              />
            ...
            <div className="space-y-2">
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <Input {...field} id="duration" placeholder="Example: 3 months / 1 month" className="bg-background" />
                )}
              />
              ...
              <div className="flex items-center gap-6">
                ...
                {isPaid && (
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => (
                      <Input {...field} type="text" placeholder="Enter amount" className="max-w-[200px]" />
                    )}
                  />
                )}
              </div>
            ...
            <div className="space-y-2">
              ...
              />
              {errors.skills_required && <p className="text-sm text-destructive">{errors.skills_required.message}</p>}
            </div>
            ...
            <div className="space-y-4">
              ...
              {languages.map((lang, index) => (
                <div key={index} className="space-y-3 p-4 border rounded-lg bg-muted/30">
                  <div className="flex items-center gap-4">
                    ...
                    {languages.length > 1 && (
                      <Button type="button" variant="ghost" size="sm" onClick={() => handleRemoveLanguage(index)}>
                        <X className="w-4 h-4" />
                      </Button>
                    )}
                  </div>
                    ...
                    <div className="flex gap-6">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id={`read-${index}`}
                          checked={lang.read}
                          onCheckedChange={(checked) => handleLanguageChange(index, "read", checked === true)}
                        />
                        <label
                          htmlFor={`read-${index}`}
                      ...
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id={`write-${index}`}
                          checked={lang.write}
                          onCheckedChange={(checked) => handleLanguageChange(index, "write", checked === true)}
                        />
                        <label
                          htmlFor={`write-${index}`}
                          className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                      ...
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id={`speak-${index}`}
                          checked={lang.speak}
                          onCheckedChange={(checked) => handleLanguageChange(index, "speak", checked === true)}
                        />
                        <label
                          htmlFor={`speak-${index}`}
                          className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
              ...
              <Button type="button" variant="link" onClick={handleAddLanguage} className="text-primary pl-0">
